  ...
  const onSubmit = async (formData: Record<string, any>) => {
    await fetch(GOOGLE_URL, {
      ...
      body: new URLSearchParams({
        [FormKeys.amount]: formData[FormKeys.amount] ?? '',
        [FormKeys.startMonth]: formData[FormKeys.startMonth] ?? '',
        [FormKeys.startYear]: formData[FormKeys.startYear] ?? '',
        [FormKeys.revenue]: String(formData[FormKeys.revenue] ?? '').replace(/[^\d]/g, ''),
        [FormKeys.creditScore]: formData[FormKeys.creditScore] ?? '',
        [FormKeys.loanFor]: formData[FormKeys.loanFor] ?? '',
        [FormKeys.zipCode]: formData[FormKeys.zipCode] ?? '',
        [FormKeys.businessName]: formData[FormKeys.businessName] ?? '',
        [FormKeys.firstName]: formData[FormKeys.firstName] ?? '',
        [FormKeys.lastName]: formData[FormKeys.lastName] ?? '',
        [FormKeys.phone]: formData[FormKeys.phone] ?? '',
        [FormKeys.email]: formData[FormKeys.email] ?? '',
      }).toString(),
    });
  };
